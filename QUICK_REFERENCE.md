...
{:port 8080                    ; Server port
 ...
 :uploads "./uploads/inv/" ; Upload directory
...
### Load Menu System
```clojure
(require '[inv.engine.menu :as menu])
...
### Load Entity Config
```clojure
(require '[inv.engine.config :as config])
(config/load-entity-config :clientes)  ; Load specific entity
```

### Test Scaffold
```clojure
(require '[inv.engine.scaffold :as scaffold])
...
:fields [{:id :email
          :type :email
          :required true
          :validator :inv.validators/check-email}]
```
...
Edit entity config:
```clojure
:hooks {:before-save :inv.hooks/calculate-total
        :after-save :inv.hooks/send-email}
```
