...
- Hook file: `src/inv/hooks/products.clj`
...
{:id :total
 :label "Total"
 :type :computed
 :compute-fn :inv.hooks.orders/calculate-total}
```
...
;; In entity config
{:id :price
 :validation :inv.validators.products/positive-price?}
```
...
Create hooks in `src/inv/hooks/`

### 6. Customize UI (if needed)
Create custom renderers in `src/inv/views/`
...
(require '[inv.engine.query :as query])
