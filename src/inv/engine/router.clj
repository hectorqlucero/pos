(ns inv.engine.router
  (:require
   [compojure.core :refer [defroutes GET POST context]]
   [ring.util.response :refer [redirect]]
   [hiccup.core :refer [html]]
   [inv.engine.config :as config]
   [inv.engine.query :as query]
   [inv.engine.crud :as crud]
   [inv.engine.render :as render]
   [inv.tabgrid.core :as tabgrid]
   [inv.models.util :refer [get-session-id user-level]]
   [inv.layout :refer [application error-404]]))
