...
(defn build-grid-require
  ...
  [table]
  (let [data (str "   [inv.handlers.admin." table ".controller :as " table "-controller]")]
    data))

(defn build-reporte-require
  "Genera archivos requeridos para un reporte para un controller"
  [table]
  (let [data (str "   [inv.handlers.reports." table ".controller :as " table "-report]")]
    data))

(defn build-require
  "Genera archivos requeridos para un dashboard para una tabla"
  [table]
  (let [data (str "   [inv.handlers." table ".controller :as " table "-dashboard]")]
    data))
