...
(defn copy-all-data
  ...
  (let [target-type (keyword (or (first args) "mysql"))
    ...
    (println (str "Source: SQLite (db/inv.db)"))
    ...
    (try
      ...
      (catch Exception e
        ...
        (println "\nMake sure:")
        (println "1. SQLite database exists at db/inv.db")
        (println "2. Target database server is running")
        (println "3. Target database credentials are correct in inv.db.migrator/db-configs")
