...
2. **Configure database connection** in `src/inv/db/migrator.clj`:
   ```clojure
   (def db-configs
     {:sqlite
...
```
→ Update password in `src/inv/db/migrator.clj`
...
Edit `src/inv/db/converter.clj` to add custom conversions:
...
To migrate specific tables only, edit `src/inv/db/migrator.clj`:
...
sqlite3 db/inv.db .dump > dump.sql
# Manually edit dump.sql to fix syntax
mysql -u root -p inv < dump.sql
...
**"Access denied"**
- Update username/password in `src/inv/db/migrator.clj`
